...
POPFILEREF=Sample_Information/ThousandGenomesSamples_AdmxRm.txt
# 5. The path to and file name of the unknown panel samples list
POPFILEUNK=Sample_Information/ThousandGenomesSamples_Admx_samples.txt
# 6. List of autosomes you are analyzing. If you are analyzing all 22 chromosomes
# it would look like the CHRLST variable below
...
if [[ $1 = "A" ]]; then
	echo "The autosomes will be analyzed."
	echo ""
	echo "All autosome files must be merged into one file. The merge list is being created."
	echo ""
	# Create the merge list
	# This command makes a text file with the file names for all of the autosome
	# VCFs generated by snakemake. The VCFs per chromosome contain both the samples
	# from the reference panel and your unknown or unverified samples.
	python make_merge_list.py --path autosomes/merge/ --chrs $CHRLST --stem _reference_panel_unknown_set_SNPs_merge_no_missing_plink_LDprune --out merge_list

	echo ""

	echo "Now that the merge list is created, the individual autosomes files will be merged to create one file with all autosomes."
	echo ""
	# Merge the plink files
	plink --file autosomes/merge/chr1_reference_panel_unknown_set_SNPs_merge_no_missing_plink_LDprune --merge-list merge_list.txt --snps-only --recode --out autosomes/merge/merge_all_chr_reference_panel_unknown_set_SNPs_merge_no_missing_plink_LDprune
	echo ""
	...
	echo "The results are now being plotted and an inferred ancestry report is being generated for all autosomes."
	echo ""
